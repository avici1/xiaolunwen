...
for (ii in 1:100) {
  hemo_cb.temp_l = crossbasis(hemo_all$temp_l, lag=ii, argvar=list(fun="ns", knots= c(1)),arglag=list(fun="poly",degree=3))
  hemo_model_all = glm(counts ~ hemo_cb.temp_l + ns(date,10*1),family=quasipoisson(), hemo_all)
...
for (ii in 1:100) {
  infar_cb.temp_l = crossbasis(infar_all$temp_l, lag=ii, argvar=list(fun="ns", knots= c(1)),arglag=list(fun="poly",degree=3))
  infar_model_all = glm(counts ~ infar_cb.temp_l + ns(date,10*1),family=quasipoisson(), infar_all)
...
for (ii in 1:30) {  
  ...
  for (jj in 1:5) {
    ...
    for ( kk in 1:5){
      ...
      hemo_cb.temp_l = crossbasis(hemo_all$temp_l, lag=ii, argvar=list(fun="poly", degree=jj), arglag=list(fun="poly", degree=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      hemo_model = glm(counts ~ hemo_cb.temp_l + ns(date, 10*1), family=quasipoisson(), data=hemo_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:30) {  
  ...
  for (jj in 1:5) {
    ...
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      hemo_cb.temp_l = crossbasis(hemo_all$temp_l, lag=ii, argvar=list(fun="ns", df=jj), arglag=list(fun="poly", degree=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      hemo_model = glm(counts ~ hemo_cb.temp_l + ns(date, 10*1), family=quasipoisson(), data=hemo_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:30) {  
  for (jj in 1:5) {
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      hemo_cb.temp_l = crossbasis(hemo_all$temp_l, lag=ii, argvar=list(fun="bs", df=jj), arglag=list(fun="poly", degree=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      hemo_model = glm(counts ~ hemo_cb.temp_l + ns(date, 10*1), family=quasipoisson(), data=hemo_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:30) {  
  for (jj in 1:5) {
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      hemo_cb.temp_l = crossbasis(hemo_all$temp_l, lag=ii, argvar=list(fun="poly", degree=jj), arglag=list(fun="ns", df=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      hemo_model = glm(counts ~ hemo_cb.temp_l + ns(date, 10*1), family=quasipoisson(), data=hemo_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:30) {  
  for (jj in 1:5) {
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      hemo_cb.temp_l = crossbasis(hemo_all$temp_l, lag=ii, argvar=list(fun="ns", df=jj), arglag=list(fun="ns", df=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      hemo_model = glm(counts ~ hemo_cb.temp_l + ns(date, 10*1), family=quasipoisson(), data=hemo_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:30) {  
  for (jj in 1:5) {
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      hemo_cb.temp_l = crossbasis(hemo_all$temp_l, lag=ii, argvar=list(fun="bs", df=jj), arglag=list(fun="ns", df=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      hemo_model = glm(counts ~ hemo_cb.temp_l + ns(date, 10*1), family=quasipoisson(), data=hemo_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:30) {  
  for (jj in 1:5) {
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      hemo_cb.temp_l = crossbasis(hemo_all$temp_l, lag=ii, argvar=list(fun="poly", degree=jj), arglag=list(fun="bs", df=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      hemo_model = glm(counts ~ hemo_cb.temp_l + ns(date, 10*1), family=quasipoisson(), data=hemo_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:30) {  
  for (jj in 1:5) {
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      hemo_cb.temp_l = crossbasis(hemo_all$temp_l, lag=ii, argvar=list(fun="ns", df=jj), arglag=list(fun="bs", df=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      hemo_model = glm(counts ~ hemo_cb.temp_l + ns(date, 10*1), family=quasipoisson(), data=hemo_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:30) {  
  for (jj in 1:5) {
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      hemo_cb.temp_l = crossbasis(hemo_all$temp_l, lag=ii, argvar=list(fun="bs", df=jj), arglag=list(fun="bs", df=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      hemo_model = glm(counts ~ hemo_cb.temp_l + ns(date, 10*1), family=quasipoisson(), data=hemo_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:30) {  
  ...
  for (jj in 1:5) {
    ...
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      infar_cb.temp_l = crossbasis(infar_all$temp_l, lag=ii, argvar=list(fun="poly", degree=jj), arglag=list(fun="poly", degree=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      infar_model = glm(counts ~ infar_cb.temp_l + ns(date, 10*1), family=quasipoisson(), data=infar_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:30) {  
  ...
  for (jj in 1:5) {
    ...
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      infar_cb.temp_l = crossbasis(infar_all$temp_l, lag=ii, argvar=list(fun="ns", df=jj), arglag=list(fun="poly", degree=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      infar_model = glm(counts ~ infar_cb.temp_l + ns(date, 10*1), family=quasipoisson(), data=infar_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:30) {  
  ...
  for (jj in 1:5) {
    ...
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      infar_cb.temp_l = crossbasis(infar_all$temp_l, lag=ii, argvar=list(fun="bs", df=jj), arglag=list(fun="poly", degree=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      infar_model = glm(counts ~ infar_cb.temp_l + ns(date, 10*1), family=quasipoisson(), data=infar_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:30) {  
  ...
  for (jj in 1:5) {
    ...
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      infar_cb.temp_l = crossbasis(infar_all$temp_l, lag=ii, argvar=list(fun="poly", degree=jj), arglag=list(fun="ns", df=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      infar_model = glm(counts ~ infar_cb.temp_l + ns(date, 10*1), family=quasipoisson(), data=infar_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:30) {  
  ...
  for (jj in 1:5) {
    ...
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      infar_cb.temp_l = crossbasis(infar_all$temp_l, lag=ii, argvar=list(fun="ns", df=jj), arglag=list(fun="ns", df=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      infar_model = glm(counts ~ infar_cb.temp_l + ns(date, 10*1), family=quasipoisson(), data=infar_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:30) {  
  ...
  for (jj in 1:5) {
    ...
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      infar_cb.temp_l = crossbasis(infar_all$temp_l, lag=ii, argvar=list(fun="bs", df=jj), arglag=list(fun="ns", df=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      infar_model = glm(counts ~ infar_cb.temp_l + ns(date, 10*1), family=quasipoisson(), data=infar_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:30) {  
  ...
  for (jj in 1:5) {
    ...
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      infar_cb.temp_l = crossbasis(infar_all$temp_l, lag=ii, argvar=list(fun="poly", degree=jj), arglag=list(fun="bs", df=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      infar_model = glm(counts ~ infar_cb.temp_l + ns(date, 10*1), family=quasipoisson(), data=infar_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:30) {  
  ...
  for (jj in 1:5) {
    ...
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      infar_cb.temp_l = crossbasis(infar_all$temp_l, lag=ii, argvar=list(fun="ns", df=jj), arglag=list(fun="bs", df=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      infar_model = glm(counts ~ infar_cb.temp_l + ns(date, 10*1), family=quasipoisson(), data=infar_all)
      # 获取 AIC 和 BIC 值
      q <- cbind("lag=",ii,"argvar=ns,df=", jj,"arglag=ns,df=",kk, qaicbic(infar_model))  
...
for (ii in 1:30) {  
  ...
  for (jj in 1:5) {
    ...
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      infar_cb.temp_l = crossbasis(infar_all$temp_l, lag=ii, argvar=list(fun="bs", df=jj), arglag=list(fun="bs", df=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      infar_model = glm(counts ~ infar_cb.temp_l + ns(date, 10*1), family=quasipoisson(), data=infar_all)
      # 获取 AIC 和 BIC 值
      q <- cbind("lag=",ii,"argvar=bs,df=", jj,"arglag=ns,df=",kk, qaicbic(infar_model))  
