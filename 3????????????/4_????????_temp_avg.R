...
for (ii in 1:100) {
  ...
  hemo_model_all = glm(counts ~ hemo_cb.temp_avg + ns(date,8*7) + weekend + holiday,family=quasipoisson(), hemo_all)
...
for (ii in 1:100) {
  ...
  infar_model_all = glm(counts ~ infar_cb.temp_avg + ns(date,8*7) + weekend + holiday,family=quasipoisson(), infar_all)
...
for (ii in 1:100) {
  stroke_cb.temp_avg = crossbasis(stroke_all$temp_avg, lag=ii, argvar=list(fun="ns", knots= c(1)),arglag=list(fun="poly",degree=3))
  stroke_model_all = glm(counts ~ stroke_cb.temp_avg + ns(date,8*7) + weekend + holiday,family=quasipoisson(), stroke_all)
...
for (ii in 1:14) {  
  ...
  for (jj in 1:5) {
    ...
    for ( kk in 1:5){
      ...
      stroke_cb.temp_avg = crossbasis(stroke_all$temp_avg, lag=ii, argvar=list(fun="poly", degree=jj), arglag=list(fun="poly", degree=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      stroke_model = glm(counts ~ stroke_cb.temp_avg + ns(date,8*7) + weekend + holiday,family=quasipoisson(), stroke_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:14) {  
  ...
  for (jj in 1:5) {
    ...
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      stroke_cb.temp_avg = crossbasis(stroke_all$temp_avg, lag=ii, argvar=list(fun="ns", df=jj), arglag=list(fun="poly", degree=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      stroke_model = glm(counts ~ stroke_cb.temp_avg + ns(date,8*7) + weekend + holiday,family=quasipoisson(), stroke_all)
      # 获取 AIC 和 BIC 值
      q <- cbind("lag=",ii,"argvar=ns,df=", jj,"arglag=poly,degree=",kk, qaicbic(stroke_model))  
...
for (ii in 1:14) {  
  for (jj in 1:5) {
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      stroke_cb.temp_avg = crossbasis(stroke_all$temp_avg, lag=ii, argvar=list(fun="bs", df=jj), arglag=list(fun="poly", degree=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      stroke_model = glm(counts ~ stroke_cb.temp_avg + ns(date,8*7) + weekend + holiday,family=quasipoisson(), stroke_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:14) {  
  for (jj in 1:5) {
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      stroke_cb.temp_avg = crossbasis(stroke_all$temp_avg, lag=ii, argvar=list(fun="poly", degree=jj), arglag=list(fun="ns", df=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      stroke_model = glm(counts ~ stroke_cb.temp_avg + ns(date,8*7) + weekend + holiday,family=quasipoisson(), stroke_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:14) {  
  for (jj in 1:5) {
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      stroke_cb.temp_avg = crossbasis(stroke_all$temp_avg, lag=ii, argvar=list(fun="ns", df=jj), arglag=list(fun="ns", df=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      stroke_model = glm(counts ~ stroke_cb.temp_avg + ns(date,8*7) + weekend + holiday,family=quasipoisson(), stroke_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:14) {  
  for (jj in 1:5) {
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      stroke_cb.temp_avg = crossbasis(stroke_all$temp_avg, lag=ii, argvar=list(fun="bs", df=jj), arglag=list(fun="ns", df=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      stroke_model = glm(counts ~ stroke_cb.temp_avg + ns(date,8*7) + weekend + holiday,family=quasipoisson(), stroke_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:14) {  
  for (jj in 1:5) {
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      stroke_cb.temp_avg = crossbasis(stroke_all$temp_avg, lag=ii, argvar=list(fun="poly", degree=jj), arglag=list(fun="bs", df=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      stroke_model = glm(counts ~ stroke_cb.temp_avg + ns(date,8*7) + weekend + holiday,family=quasipoisson(), stroke_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:14) {  
  for (jj in 1:5) {
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      stroke_cb.temp_avg = crossbasis(stroke_all$temp_avg, lag=ii, argvar=list(fun="ns", df=jj), arglag=list(fun="bs", df=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      stroke_model = glm(counts ~ stroke_cb.temp_avg + ns(date,8*7) + weekend + holiday,family=quasipoisson(), stroke_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:14) {  
  for (jj in 1:5) {
    for ( kk in 1:5){
      # 创建交叉基函数，其中 lag 由外部循环 jj 控制
      stroke_cb.temp_avg = crossbasis(stroke_all$temp_avg, lag=ii, argvar=list(fun="bs", df=jj), arglag=list(fun="bs", df=kk))
      # 拟合广义线性模型，使用 quasipoisson 分布
      stroke_model = glm(counts ~ stroke_cb.temp_avg + ns(date,8*7) + weekend + holiday,family=quasipoisson(), stroke_all)
      # 获取 AIC 和 BIC 值
...
for (ii in 1:14) {  
  ...
  for (jj in 1:5) {
    ...
    for ( kk in 1:5){
      ...
      # 拟合广义线性模型，使用 quasipoisson 分布
      hemo_model = glm(counts ~ hemo_cb.temp_avg + ns(date,8*7) + weekend + holiday,family=quasipoisson(), hemo_all)
      # 获取 AIC 和 BIC 值
