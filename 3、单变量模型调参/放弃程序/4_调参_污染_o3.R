...
for (ii in 1:100) {
  ...
  hemo_model_all = glm(counts ~ hemo_cb.o3 + ns(date,10*1),family=quasipoisson(), hemo_all)
...
for (ii in 1:100) {
  ...
  infar_model_all = glm(counts ~ infar_cb.o3 + ns(date,10*1),family=quasipoisson(), infar_all)
...
for (ii in 1:30) {  
  ...
  for (jj in 1:5) {
    ...
    for ( kk in 1:5){
      ...
      hemo_model = glm(counts ~ hemo_cb.o3 + ns(date, 10*1), family=quasipoisson(), data=hemo_all)
      # 获取 AIC 和 BIC 值
